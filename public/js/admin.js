...
let allMessagesCache = []; // שמירת ההודעות כדי לאפשר חיפוש מהיר
...
async function loadMessages() {
    ...
    try {
        ...
        const data = await response.json();
        allMessagesCache = data.messages || []; // שומרים בזיכרון לטובת החיפוש
...
function renderMessages(messagesToRender) {
    ...
    if (Object.keys(grouped).length === 0) {
        container.innerHTML = '<p class="status-msg">אין שיחות התואמות לחיפוש</p>';
        return;
    }
